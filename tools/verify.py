...
with sync_playwright() as p:
    ...
    note(op == '1' and focused == 'Home', f'menu opens, focus on the first link ({focused!r})')
    page.keyboard.press('Escape')
    page.wait_for_timeout(500)
    note(page.evaluate("document.activeElement.getAttribute('aria-label')") == 'Open menu', 'Escape closes the menu and returns focus to the button')
    page.close()
...
note(all(os.path.exists(os.path.join(ROOT, 'dist', f)) for f in ('404.html', 'sitemap.xml', 'robots.txt')), '404.html, sitemap.xml, robots.txt exist')
