...
const NONE: [string, string][] = [
  ...
  ['Readable source', 'What your browser downloads is everything there is. Each parser is a few hundred lines; the network panel confirms what they do.'],
]
...
const MODEL: [string, string][] = [
  ...
  ['The formats not yet read', 'Not protected. Video, GIF, TIFF, PSD, legacy Office, and the pictures pasted inside a Word document keep their metadata. The page refuses what it cannot read rather than pretend.'],
  ['PDF compressed streams', 'Partly protected. Info and uncompressed XMP are blanked; compressed XMP and object streams are shown and kept, and the result says when that has happened.'],
    ...
    <>
      ...
      <Section labelledBy="h-none">
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {NONE.map(([t, b], i) => (
            <div key={t} className="bezel" data-reveal="" style={{ '--d': `${i * 50}ms` } as React.CSSProperties}>
              <div className="plate h-full p-6">
                <h3 className="text-[1.125rem]">{t}</h3>
                <p className="mt-2 text-[0.9375rem] text-muted">{b}</p>
              </div>
            </div>
          ))}
        </div>
