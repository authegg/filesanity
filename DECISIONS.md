...
- 2026-09-20 later: navigations switched from stale-while-revalidate to network-first with a 3 s cache fallback. The owner saw the old pricing copy after a deploy: SWR shows the previous page once per edit, which is wrong for a site whose copy changes. Rejected: keeping SWR with an update toast (a reload prompt on a marketing page).

## Critic round 3 (FAIL 4), 2026-09-20, after v1.1

Q1 caption: "A single white rounded panel fills the top of a grey page, with the headline top-left and a dashed drop box to its right; below it the grey page runs through three white product-still cards, a numbered three-step list, a six-row format table, a glass-cube still, and a red-outlined closing band with a button." Q1, Q2, Q3, Q4, Q6 PASS; Q5 FAIL on four items.

1. Footer: four columns (Product, Tools, What files carry, Company) against the logged three. Kept at four and logged: the Tools column is five downloads and a source listing, a different kind of link from the pages; folded into Product it makes a ten-item column, and cut it hides the things v1.1 exists to ship. Rejected: three columns with Tools inside Product (ten items), a second footer row (a second footer).
2. Tool pages (/batch, /cli, /api, /extension), surface calls now logged:
   - Hero: the inner-page PageHead shared by every non-home page (eyebrow, one-line headline, lede), then one bezel panel. Rejected: a per-tool hero with the object (a terminal, a browser window) as the picture; the studio bans fake terminals and product screenshots, and the tool is a download, not a fold.
   - The panel is hairline rows with the command in mono left and a plain annotation right: anchor 1's spec table (the manual's mono rows), one row per command. Rejected: a syntax-highlighted code block with comments (a fake terminal), prose paragraphs with inline code (the reader cannot copy a line).
   - /extension's numbered steps mirror the home page's three-step list (Drop, Read, Download); the same device for the same shape of content. Rejected: three cards.
   - /batch's policy is native checkboxes with `accent-color`: the brief's custom-control rule is about the file input, which cannot be styled; a checkbox can, and the native one is keyboard, screen-reader and 320 px correct for free (ladder rung 4). Rejected: twelve custom pill toggles (a second control language on one page).
   - The filled pill plus text link pair beside each download is the home hero's own pair (pill "Clean a file" plus link "How it works"), used once per tool page. Rejected: two pills (two primaries), a list of links (no primary).
3. Eyebrows cut to one per page on the four tool pages: the section eyebrows (Options, Contract, What it does and does not, How the zip is made) removed; the policy legend is set as a plain medium line, not an eyebrow.
4. Negation ledes reworded on /batch and /cli, and the home closer, to say what happens rather than what does not. The home hero's sub-line keeps the brief's fixed statement, logged in v2. Rejected: keeping "Nothing is uploaded" on every tool page (the same sentence five times is a tic, and the fixed idea already sits on the home page and the security page).
Also from the critic's notes: the field table now lists position, names, device identity and dates before exposure settings within each part (the rows a stranger fears were rows 12 to 40); the Contact page no longer prints a source path; the legal note reads "not yet read by a lawyer" instead of "before launch" on a site that has launched; the two browse links on /batch lost their icons (the only icon-plus-link controls on the site).
Not changed: the empty left half of the home tray at 1917 wide (the tray is the page; filling it is decoration), the section gap above the tool panels (the same gap as every inner page), the batch File column width (names are the thing to read).
...
- Blog stills (2026-09-20): five Codex stills in the v2 style clause (silver surface, diffused light upper left, white and charcoal, one dark red detail, no text, no people), one object per post chosen as the post's noun: an SD card (JPEG), a document folder with a red band (Office), a stapled sheaf (PDF), a sealed envelope (the file that does not leave), a blank luggage tag (sending to a stranger). AVIF q55 at 1536 and 768, 4 to 16 kB. Post page: the still in a bezel plate under the head at 3:1 (2:1 on phones), the body in the 68ch prose column below. Index: a 3:2 thumbnail in a bezel per row, the title is the link. Rejected: Higgsfield (the three existing stills are Codex and the set must read as one shoot), a still inside the body column (a magazine, not a manual), no images (the critic named the bare grey post body).

## Cloud API (2026-09-20)

- The owner chose to monetise through Lemon Squeezy with the hosted API, after a three-voice debate (buyer, operator, privacy lead; transcript in the session's workflow output) recommended Teams-first and no hosted API. Owner's call; the panel's objections are answered where they can be: the hosted service is on its own origin (api.filesanity.com), its own page and its own terms, the main site keeps every "no server" sentence true, and the free path (self-host, MIT) is named on the cloud page before the plans. Rejected: refusing to build it (the owner asked twice), and a hosted tier on the main pricing page's Free plate (it would make "no server behind the page" false).
- Keys: trial keys issued by the Worker against an email (500 files once, 25 MB, one per address, three per network address a day); paid keys are Lemon Squeezy licence keys validated with its API and cached one hour, cleared by its licence webhooks. Rejected: our own accounts and passwords (a database and a login flow for a one-person shop), Lemon Squeezy's customer portal as the key store (an API call per request).
- Counting in KV, one key per key per month, with a 40-day TTL. Not atomic. ponytail: a Durable Object per key is the upgrade if two requests in the same millisecond ever matter for billing.
- Signed clean-reports: Ed25519 over a JSON of counts and the SHA-256 of the clean bytes, public key at /.well-known/filesanity-report-key. Rejected: unsigned JSON (nothing a third party can check), a PDF certificate (a template to design and no more truth in it).
- The cloud page's three plates for three plans are the pricing-table case, not three feature cards; each has its own primary. Rejected: one comparison table (on a phone a four-column table stacks worse than three plates, and the trial's action differs from the two subscriptions).
- The cloud page gets its own terms and privacy at /cloud/terms, and the main privacy policy and About name it as the one exception. Rejected: folding it into the main policy (the main policy's first sentence would stop being true).
- Prices: Starter $19 for 10,000 files, Business $79 for 100,000, both 100 MB; trial 25 MB. Set from metacleaner's EUR30 Advance (bandwidth-metered, uploads) and the panel's numbers; the owner can change them in src/cloud.ts, which is the only place they live.
